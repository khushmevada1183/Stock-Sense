...
const PortfolioDashboard = () => {
  ...
  return (
    <div ref={dashboardRef} className="space-y-8">
      ...
      <div className="dashboard-card bg-gray-900 rounded-xl p-6 shadow-lg">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-white">{formatCurrency(summary.totalValue)}</h1>
            <div className="flex items-center mt-2">
              <span className={`text-lg font-medium ${summary.totalProfitLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="mt-4 md:mt-0">
            <div className="flex items-center">
              ...
              <span className={`font-medium ${summary.dayGain >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <div className="flex items-center mt-1">
              ...
              <span className={`font-medium ${summary.totalProfitLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Risk Profile Card */}
        <div className="dashboard-card bg-gray-900 rounded-xl p-6 shadow-lg">
          <h3 className="text-lg font-serif text-gray-300 mb-4">Risk Profile</h3>
          <div className="space-y-4">
            ...
            <div className="relative h-3 bg-gradient-to-r from-green-500 via-yellow-500 to-red-500 rounded-full">
              <div 
                className="absolute h-5 w-5 bg-white rounded-full -mt-1 transform -translate-x-1/2 border-4 border-gray-900"
                style={{ 
        ...
        {/* Valuation Card */}
        <div className="dashboard-card bg-gray-900 rounded-xl p-6 shadow-lg">
          <h3 className="text-lg font-serif text-gray-300 mb-4">Valuation</h3>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="text-center">
                <span className="block text-2xl font-bold text-white">{summary.valuationScore}</span>
        ...
        {/* Diversity Card */}
        <div className="dashboard-card bg-gray-900 rounded-xl p-6 shadow-lg">
          <h3 className="text-lg font-serif text-gray-300 mb-4">Diversity</h3>
          <div className="flex items-center justify-center">
            <div className="relative w-32 h-32">
              {/* Simple donut chart */}
              <svg viewBox="0 0 100 100" className="w-full h-full">
                <circle cx="50" cy="50" r="40" fill="transparent" stroke="#3B82F6" strokeWidth="20" strokeDasharray={`${summary.sectorAllocation[0].percentage * 2.51} 251`} transform="rotate(-90 50 50)" />
                <circle cx="50" cy="50" r="40" fill="transparent" stroke="#A855F7" strokeWidth="20" strokeDasharray={`${summary.sectorAllocation[1].percentage * 2.51} 251`} strokeDashoffset={`${-summary.sectorAllocation[0].percentage * 2.51}`} transform="rotate(-90 50 50)" />
      ...
      {/* Holdings Table */}
      <div className="dashboard-card bg-gray-900 rounded-xl p-6 shadow-lg overflow-x-auto">
        <h3 className="text-lg font-mono font-bold text-gray-300 mb-4">Your Holdings</h3>
          ...
          <tbody>
            {holdings.map((stock) => (
              <tr key={stock.symbol} className="border-b border-gray-800 hover:bg-gray-800/50">
                <td className="py-4">
                  <Link href={`/stocks/${stock.symbol}`} className="text-blue-400 hover:text-blue-300 font-medium">
                    {stock.symbol}
                  </Link>
                </td>
                ...
                <td className={`py-4 ${stock.profitLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {stock.profitLoss >= 0 ? '+' : ''}{formatCurrency(stock.profitLoss)}
                </td>
                <td className={`py-4 ${stock.profitLossPercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
