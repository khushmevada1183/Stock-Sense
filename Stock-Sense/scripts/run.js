...
if (isCloudDeployment) {
  // In cloud environments, the provider's PORT should be used for the main service
  // Render assigns a PORT value that should be used by the primary service
  backendPort = parseInt(process.env.PORT, 10) || 5005;
  
  // For multi-service deployments, the frontend would have its own service
  // and receive its own PORT value from Render
  frontendPort = parseInt(process.env.PORT, 10) || 3000;
  
  console.log(`Cloud deployment detected (${isCloudDeployment}). Using assigned ports.`);
  console.log(`Backend port: ${backendPort}, Frontend port: ${frontendPort}`);
  
  // When running locally but simulating cloud, we need different ports
  if (backendPort === frontendPort && !process.env.RENDER_INTERNAL_SERVICE) {
    console.log('Adjusting frontend port to avoid conflict in local simulation');
