...
function getApiClient(): ApiClient {  
  // Get API URL from environment variables or localStorage with more robust fallback
  let apiUrl = '';
  
  if (typeof window !== 'undefined') {
    // Client-side: try localStorage first, then env var, then fallback
    apiUrl = localStorage.getItem('api_url') || 
            process.env.NEXT_PUBLIC_API_URL || '';
    
    // For Render deployment, if the URL doesn't include protocol, add it
    if (apiUrl && !apiUrl.startsWith('http')) {
      apiUrl = `https://${apiUrl}`;
    }
    
    // If NEXT_PUBLIC_API_URL is set to a Render service URL, ensure it has /api path
    if (apiUrl && apiUrl.includes('onrender.com') && !apiUrl.endsWith('/api')) {
      apiUrl = `${apiUrl}/api`;
    }
    ...
    // Same protocol and path handling for server-side
    if (apiUrl && !apiUrl.startsWith('http')) {
      apiUrl = `https://${apiUrl}`;
    }
  ...
  console.log(`API client configured with base URL: ${apiUrl}`);
